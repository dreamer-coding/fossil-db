...
FOSSIL_TEST(c_test_bluecrab_create_open_close_delete)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_create(TEST_DB_PATH, TEST_DB_NAME) == 0);

    fossil_bluecrab_db db;
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_open(&db, TEST_DB_PATH) == 0);
    ASSUME_ITS_TRUE(db.opened);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_close(&db) == 0);

    // Ensure file is closed before attempting to delete, and retry if needed
    int del_result = fossil_db_database__bluecrab_delete(TEST_DB_PATH);
    if (del_result != 0) {
        // Wait briefly and try again in case of OS file lock delay
        FOSSIL_SANITY_SYS_SLEEP(1000); // 1 second
        del_result = fossil_db_database__bluecrab_delete(TEST_DB_PATH);
    }
...
FOSSIL_TEST(c_test_bluecrab_crud_entry)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, id, fson) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, id, &out) == 0);
    ASSUME_NOT_CNULL(out);
    ASSUME_ITS_CSTR_CONTAINS(out, "Alice");
    free(out);

    const char *fson2 = "object: { name: cstr:\"Bob\", age: i32:40 }";
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_update(&db, id, fson2) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, id, &out) == 0);
    ASSUME_NOT_CNULL(out);
    ASSUME_ITS_CSTR_CONTAINS(out, "Bob");
    free(out);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_remove(&db, id) == 0);
    ASSUME_NOT_TRUE(fossil_db_database__bluecrab_get(&db, id, &out) == 0);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_close(&db) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_delete(TEST_DB_PATH) == 0);
}

FOSSIL_TEST(c_test_bluecrab_subentry)
{
    FOSSIL_SANITY_SYS_DELETE_FILE(TEST_DB_PATH);
    fossil_db_database__bluecrab_create(TEST_DB_PATH, TEST_DB_NAME);
    fossil_bluecrab_db db;
    fossil_db_database__bluecrab_open(&db, TEST_DB_PATH);
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert_sub(&db, parent, sub, fson) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get_sub(&db, parent, sub, &out) == 0);
    ASSUME_NOT_CNULL(out);
    ASSUME_ITS_CSTR_CONTAINS(out, "123");
    free(out);

    fossil_db_database__bluecrab_close(&db);
    fossil_db_database__bluecrab_delete(TEST_DB_PATH);
}
...
FOSSIL_TEST(c_test_bluecrab_relations)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "A", "object: { }") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "B", "object: { }") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "C", "object: { }") == 0);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_link(&db, "A", "B", "friend") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_link(&db, "A", "C", "colleague") == 0);

    fossil_bluecrab_relation *rels = NULL;
    size_t count = 0;
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get_relations(&db, "A", &rels, &count) == 0);
    ASSUME_ITS_MORE_OR_EQUAL_SIZE(count, 2);
    free(rels);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_unlink(&db, "A", "B") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get_relations(&db, "A", &rels, &count) == 0);
...
FOSSIL_TEST(c_test_bluecrab_hash_and_verify)
{
    ...
    fossil_db_database__bluecrab_insert(&db, id, fson);

    char *data = NULL;
    char hash[FOSSIL_BLUECRAB_HASH_SIZE];
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, id, &data) == 0);
    ASSUME_NOT_CNULL(data);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_hash_entry(data, hash) == 0);
    free(data);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_verify_entry(&db, id) == 0);
...
FOSSIL_TEST(c_test_bluecrab_commit_log_checkout)
{
    ...
    fossil_db_database__bluecrab_insert(&db, "c1", "object: { v: i32:1 }");
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_commit(&db, "Initial commit") == 0);

    fossil_db_database__bluecrab_insert(&db, "c2", "object: { v: i32:2 }");
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_commit(&db, "Second commit") == 0);

    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_log(&db) == 0);

    // Try checkout (should print commit info)
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_checkout(&db, "1") == 0);
...
FOSSIL_TEST(c_test_bluecrab_meta_and_advanced)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_meta_load(&db) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_meta_save(&db) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_meta_rebuild(&db) == 0);
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_backup(&db, backup_path) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_restore(&db, backup_path) == 0);

    // Compact and verify
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_compact(&db) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_verify(&db) == 0);
...
FOSSIL_TEST(c_test_bluecrab_multiple_entries_and_bulk_crud)
{
    ...
    char *out = NULL;
    for (int i = 0; i < 4; ++i) {
        ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, ids[i], fsons[i]) == 0);
        ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, ids[i], &out) == 0);
        ASSUME_NOT_CNULL(out);
    ...
    // Remove all entries
    for (int i = 0; i < 4; ++i) {
        ASSUME_ITS_TRUE(fossil_db_database__bluecrab_remove(&db, ids[i]) == 0);
    }

    // Confirm removal
    for (int i = 0; i < 4; ++i) {
        ASSUME_NOT_TRUE(fossil_db_database__bluecrab_get(&db, ids[i], &out) == 0);
    }
...
FOSSIL_TEST(c_test_bluecrab_subentry_update_and_remove)
{
    ...
    // Remove subentry
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_remove(&db, id) == 0);
    ASSUME_NOT_TRUE(fossil_db_database__bluecrab_get_sub(&db, parent, sub, &out) == 0);
...
FOSSIL_TEST(c_test_bluecrab_relation_metadata_and_types)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "X", "object: { }") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "Y", "object: { }") == 0);

    // Link with different relation types
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_link(&db, "X", "Y", "parent") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_link(&db, "Y", "X", "child") == 0);

    fossil_bluecrab_relation *rels = NULL;
    size_t count = 0;
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get_relations(&db, "X", &rels, &count) == 0);
    ASSUME_ITS_MORE_OR_EQUAL_SIZE(count, 2);
...
FOSSIL_TEST(c_test_bluecrab_commit_and_checkout_invalid_version)
{
    ...
    fossil_db_database__bluecrab_insert(&db, "v1", "object: { v: i32:10 }");
    fossil_db_database__bluecrab_commit(&db, "Commit 1");

    // Try to checkout a non-existent version
    ASSUME_NOT_TRUE(fossil_db_database__bluecrab_checkout(&db, "9999") == 0);
...
FOSSIL_TEST(c_test_bluecrab_hash_consistency)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, id, &data1) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_hash_entry(data1, hash1) == 0);

    // Hash again, should be the same
    char hash2[FOSSIL_BLUECRAB_HASH_SIZE];
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_hash_entry(data1, hash2) == 0);
...
FOSSIL_TEST(c_test_bluecrab_dag_cycle_prevention)
{
    ...
    // Create a DAG: A -> B -> C
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_link(&db, "A", "B", "edge") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_link(&db, "B", "C", "edge") == 0);

    // Attempt to create a cycle: C -> A (should fail)
    ASSUME_NOT_TRUE(fossil_db_database__bluecrab_link(&db, "C", "A", "edge") == 0);
...
FOSSIL_TEST(c_test_bluecrab_subentry_bulk_and_removal)
{
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, parent, "object: { }") == 0);
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert_sub(&db, parent, subid, fson) == 0);
    }
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        ASSUME_ITS_TRUE(fossil_db_database__bluecrab_remove(&db, fullid) == 0);
    }
    ...
    for (int i = 0; i < 20; ++i) {
        snprintf(subid, sizeof(subid), "sub_%02d", i);
        ASSUME_NOT_TRUE(fossil_db_database__bluecrab_get_sub(&db, parent, subid, &out) == 0);
    }
...
FOSSIL_TEST(c_test_bluecrab_backup_restore_integrity)
{
    ...
    // Insert entries
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "bk1", "object: { foo: cstr:\"bar\" }") == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_insert(&db, "bk2", "object: { foo: cstr:\"baz\" }") == 0);
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_remove(&db, "bk2") == 0);
    char *out = NULL;
    ASSUME_NOT_TRUE(fossil_db_database__bluecrab_get(&db, "bk2", &out) == 0);

    // Restore from backup and verify both entries exist
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_restore(&db, backup_path) == 0);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, "bk1", &out) == 0);
    free(out);
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_get(&db, "bk2", &out) == 0);
    free(out);
...
FOSSIL_TEST(c_test_bluecrab_similarity_and_ranking)
{
    ...
    float sim1 = fossil_db_database__bluecrab_similarity("Alpha", "Alpha");
    float sim2 = fossil_db_database__bluecrab_similarity("Alpha", "Alfa");
    float sim3 = fossil_db_database__bluecrab_similarity("Alpha", "Beta");
    ...
    ASSUME_ITS_TRUE(fossil_db_database__bluecrab_rank_results(results, 3) == 0);
